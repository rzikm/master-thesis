...
% Abstracts are stored in separate files so that they can be included also in separate PDFs
...
\begingroup
\def\par{^^J} % par macro must not appear in the Subject, so we have to replace it by newline
