...
\todo{make it more explicit that the implementation uses 64bit window}
...
\todo{another figure explaining the following paragraph, use concrete packet numbers in the explanation}
...
\begin{enumerate}
  ...
  \item add padding to the packet if necessary\footnote{QUIC requires valid QUIC packets to be at least 21 bytes.\todo{link to specification}},
...
\todo{check vertical spacing for multiline function signatures}
...
\todo{mention that following listing is in C}
...
\todo{mention the ``inspection'' word in the preceding paragraph}
