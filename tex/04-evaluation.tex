...
\begin{itemize}
        ...
        very high~\cite{Treat2015}. \todo{mention that it has very high chance to affect full page
        load}
...
% in tests with increased message size (\autoref{fig:04-multi-stream-latency-b}), or greater number of
% streams (\autoref{fig:04-multi-stream-latency-c}) or both (\autoref{fig:04-multi-stream-latency-d}),
% especially with large number of connections.
...
\todo{influence of TLS record size? (CPU overhead)}
...
\autoref{fig:04-loss-throughput} shows the measured throughput for 0.004\% packet loss
...
As for the actual results, the \libmsquic{}-based
...
\begin{myFigure}{fig:04-lab-multi-stream-latency-median}{Multiple stream QUIC latency measurements (median)}
...
\todo{calculate theoretical maximum throughput of managed impl based on latency}
...
\todo{increased latency is caused by the fact that the message does not fit into a single packet,
  the packet pacer spreads the packets out}
