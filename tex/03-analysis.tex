...
when the method completes, the provided memory can be reused. A single-copy approach in this case
would require that the \method{Write} and \method{WriteAsync} methods complete only after the peer
acknowledges that the contained data were received. In the best-case scenario would be a full
round-trip time after the method call which would be very inefficient. Therefore our implementation
will, as with the receiving part of the stream, use an intermediate buffer for storing the data.
...
writing this thesis, there already exists project for textit{System.Net.Quic.dll}. Therefore, the
