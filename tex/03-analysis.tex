...
\todo{slim this section, explain the main concepts, maybe diagrams, and move this text to user documentation}
...
\todo{add figures similar to the previous section. one for accessing the connections (extends the
  previous one), and another for accessing the streams in the connection}
...
\todo{Separate section for unit testing, because now we are introducing ShouldHaveFrame and other
helper testing methods, that is too much stuff for this section}
...
the \RecoveryController{} will use the strategy pattern \todo{should I reference some source? YES}
to allow chosing the congestion control algorithm at runtime.

\todo{try finding and mention articles that experiment with congestion control, so that we dont
  prepare for ``hypothetical future'', also we can mention that this is already done for TCP}
