...
\todo{List which original goals imply these}
...
\begin{itemize}
    ...
    \litem{Avoid excessive heap allocations} Although heap allocation is considered cheap, frequent
    \todo{rephrase? allocation is cheap, however, price is paid during garbage collection ....}
...
\todo{emphasise that this API was designed by the .NET team and was already present there, we are only fulfilling the API}
...
from the abstract \class{Stream} class. The list below mentions only members specific for the
\class{QuicStream} class, members from the \class{Stream} base class are omitted for brevity.
\todo{mention that the omitted members include Read and ReadAsync}
...
\begin{description}
    ...
    \ditemmethodWithComment{ValueTask}{WriteAsync}{*, CancellationToken}{multiple overloads} Multiple overloads of this method offer writing from various types of buffers: \class{ReadOnlyMemory<byte>}, \class{ReadOnlySequence<byte>}, and \class{ReadOnlyMemory\allowbreak<ReadOnlyMemory<byte>>} \todo{mention scatter/gather IO, 2:14m}. The returned task completes when the provided data have been buffered internally and the buffers can be reused for other purposes.
...
multiple QUIC connections. \todo{reedit following sentence} Also, during connection migration on
...
\todo{explain more: how it fits into the architecture}
...
\todo{really! mention future support for connection migration?}
...
\todo{if I don't manage to properly implement this part, then following paragraph needs to be expanded: 2020-10-22 27m}
