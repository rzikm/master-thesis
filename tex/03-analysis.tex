...
\todo{check the explanation why waiting for acks is bad}

A single-copy implementation approach in this case would require that the \method{Write} and
\method{WriteAsync} methods complete after the peer acknowledges that the data were received. In
...
would be spent waiting for acknowlegement instead of sending more data. In order to fully saturate
the available bandwidth, users of the \QuicStream{} would have to either provide very large data
buffers or use the \method{WriteAsync} method and overlap the its execution by maintaining multiple
