...
\method{ReadAsync} method, there is no application buffer to deliver into.
...
the data are delivered to the application. Tracking of which buffers can be reused again for
receiving next QUIC packets can be complicated because QUIC packets can carry multiple \STREAM{}
frames.
...
needed at any given point in time. Therefore, our implementation uses a list multiple smaller
buffers and allocates only the necessary number of buffers that are needed to buffer currently
received data. To reduce pressure on the garbage collector, buffers are pooled in a shared instance
of \ArrayPoolOf{\Byte{}} class to avoid their frequent allocation.
...
using the \ValueTaskOf{} type.

\todo{Check readability there}

Each time data are received for the stream, the implementation checks if there is a contiguous block
of memory which could be delivered to the application. For this reason, the implementation needs to
keep track which parts of the stream have already been received. If there is a new part of the
...
delivered, the intermediate buffer is returned back to the \ArrayPoolOf{\Byte{}}.
...
delivered to the application. However, sending ipdated limits in every packet could waste space that
...
\todo{improved clarity why returning from write once the data is buffered}

A single-copy implementation approach in this case would require that the \method{Write} and
\method{WriteAsync} methods complete only after the peer acknowledges that the contained data were
received. In case no packet gets lost, that implies a full round-trip delay. If we were to choose
this approach for the implementation, it would be hard to use the the \QuicStream{} class
efficiently, and the \QuicStream{} implementation would be inconsistent with the other
network-enabled \Stream{} implementations.

Therefore, our implementation will create an intermediate copy of the data and the \method{Write}
and \method{WriteAsync} will complete once the data is internally buffered. In order to be
memory-efficient, our implementation will, as in \ReceiveStream{} implementation, use an
\ArrayPoolOf{\Byte{}} to reduce pressure on the garbage collector.
...
\todo{adjusted figure and description}
...
\ArrayPoolOf{\Byte{}}. When the buffer is full or the stream is flushed, the buffer is enququed into
...
acknowleged, the buffer can be removed and returned back to the \ArrayPoolOf{\Byte}.
