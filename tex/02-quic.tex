...
\begin{itemize}
  ...
  \litem{X (i)} \todo{it is not certain that i is not a placeholder. maybe explicitly state X followed by (i).} Indicates that X is encoded using the QUIC's variable-length encoding. This encoding is described in \autoref{sec:02-variable-length-encoding}.
...
\todo{don't use hexadecimals for this, use bit constants (00)}
...
\todo{don't use the bit numbers, because now it seems that e.g. Version is also 8 bits}
...
\todo{Use these also in the list later}
...
The migration istelf is initiated sIf the path is validated, the endpoint can migrate the connection
by starting to send packets from the mew local address. After server receives the first non-probing
packet from the new client's address, it starts sending all future packets to that new address.
...
\todo{again, use bits instead of hexadecimal constants}
...
\todo{redraw with the same notation as other datagram flows}
...
confidentiality level as conventional TLS connections. \todo{backreference to the handshake flow, where Crypto frames were used}
