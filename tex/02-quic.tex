...
\begin{description}
  ...
  \ditem{\Gls{quic-packet}}  A complete processable unit of QUIC that can be
  ...
  \ditem{\Gls{network-path}} An imaginary path between two network addresses.

  \ditem{\Gls{cid}}  An opaque identifier that is used to identify a QUIC
...
\begin{itemize}
  ...
  \litem{X (i)} Literal \textit{i} indicates that X is encoded using the QUIC's variable-length encoding. This encoding is described in \autoref{sec:02-variable-length-encoding}.

  \litem{X (A..B)} Indicates that X can be any length from A to B.

  \litem{X (..)} Indicates that X can be any length. The length is generally stored in different field.
...
\begin{enumerate}
  ...
  \litem{Retry} optionally used by servers for Client Address Validation  when establishing new connections. Address validation is described in \autoref{sec:02-address-validation};
...
\gls{scid} and \gls{dcid} which are \glspl{cid} used to match the packets to the correct
...
\begin{description}
    ...
    \ditem{Fixed Bit (F)} A bit that is always set to 1 in valid QUIC packets.

    \ditem{Packet Type (T)} Discriminator of the packet type. the possible values are listed in \autoref{tab:02-packet-type}.
...
Connection IDs can be also retired by the other endpoint using the \RETIRECONNECTIONID{} frame. By
...
ID in the \gls{dcid} field, the \gls{cid} is used to find an existing connection. If the packet
...
from client connections. Server maintains a table mapping between \gls{dcid}s corresponding
...
confidentiality and authenticity of the encrypted data. In addition to encrypted data --- called
\textit{ciphertext} --- AEAD encryption outputs also an authentication tag which is used to check the
integrity of the payload during decryption. The encryption can be authenticated by supplying
additional authentication data (AAD for short) which are not encrypted, but influence the
authentication tag and, therefore, must be supplied also during decryption. As an additional
...
\todo{do something about the vertical spacing there, perhaps split the figure into 2 separate figures}
...
\begin{myFigure}{fig:02-protected-fields}{Fields protected by payload encryption and header protection}
  ...
  \todo{check baseline of the cells}
