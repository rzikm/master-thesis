...
\todo{Bit headers were removed, so there should be less confusion now.}
...
connection to a new network path. QUIC requires that the same Connection ID be used only for packets
exchanged between one pair of local and remote endpoint addresses (referred to as \textit{network
  path}). When migrating a connection, the endpoints must start using different Connection ID for
sending packets to prevent correlation of the network traffic by external observers.
...
\todo{mention that there must be a reserve of connection IDs on the client in order to allow migration (how does client get S2), also backreference stuff about issuing connection ids.}
...
refer to the same connection. QUIC requires that each Connection ID be used for only one combination
of local and remote endpoint addresses (referred to as \textit{network path}). Therefore, when
connection is migrated, both endpoints must switch to using different Connection IDs. Connection
migration can, therefore, only be initiated if both endpoints issued additional Connection IDs using
the \NEWCONNECTIONID{} frames for the other endpoint to use.
