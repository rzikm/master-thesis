...
achieved by putting the libraries in any following location:
...
created in previous step is used when building the user code. For this, following environment
variables need to be defined correctly.
...
one locally built from the \dotnet{} runtime sources.
...
on the server side. The current QUIC API requires that the certificate and the private key be saved
in separate files. A certificate that can be used in this example is provided in the attachments
\todo{path in the attachments}. Alternatively, a new certificate can be created using the
...
\begin{itemize}
  ...
  \item identifier of the application-layer protocol to be used\footnote{QUIC is designed not to be
        ...
        and \todo{emphasized that the ``echo'' choice was arbitrary by us}
...
We can then use the \method{AcceptConnectionAsync} method to asynchronously wait for new incoming
...
In this thesis, we developed a more complex version of the echo server example from previous section
for benchmarking purposes. We will provide more information about the application in
\autoref{sec:04-benchmark-app}.
