...
\todo{DONE: mention that the app will only run if the SDK is available in the environment; also
  mention self-contained publish}
...
\todo{DONE: mention that we won't handle errors. see above, also added footnote to ALPN}
...
\todo{path in the attachments}. \todo{creating a new ssl certificate using openssl cli? - could be,
...
This thesis developed a more complex version of the echo server example from previous section for
benchmarking purposes. \autoref{sec:04-benchmark-app} provides more information about the
application.

\todo {see recording: 47:00, 2020-12-08}
