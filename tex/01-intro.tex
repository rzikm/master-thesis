...
used by web browsers to download \todo{use different word} and otherwise interact with websites, it also serves as a transport
medium for use with REST web APIs or popular technologies such as gRPC and GraphQL\@.

Currently, the latest published version of the protocol is HTTP/2 defined in RFC 7540 from 2015~\todo{citations to references}.
HTTP/2 improved on its predecessor HTTP/1.1 \todo{also add reference} by introducing features such as request multiplexing
over a single TCP connection, header compression and the server-push feature, allowing it to
significantly reduce loading times of web pages and generally improve the efficiency of the web \todo{optionally also try to find a reference to measurements supporting this claim}.

However, HTTP/2 protocol performance is still limited in some scenarios and some limiting
factors can be traced to the TCP protocol used on the transport layer. \todo{add reference to source of the analysis? Who found out where is the bottleneck of HTTP/2?} Combination of request
...
requests. This problem is generally known as \textit{head-of-line blocking}\todo{add reference to wikipedia or other source which defines the term}.

Another area where HTTP/2 protocol can be improved is the connection establishment latency. For
security reasons, almost all web connections now use HTTPS instead of plain HTTP\@. \todo{HTTP/2 is an extension which still allowed unencrypted HTTP connection, but today there is a strong pressure to use encrypted connections. Mention that HTTPS uses TLS as an 'nadstavba'. When designing HTTP/3, it can be assumed that encryption will be required and used at almost all times}
...
actual HTTP request can be sent. \todo{maybe an illustration would be useful}

The above mentioned and other problems \todo{reword before this, sounds weird, it seems that there is a word missing after `mentioned'} are addressed in the HTTP/2's successor HTTP/3 \todo{add references to HTTP/3, } \todo{mention HTTP/3 first to put more epmhasis on it}. At the time
of writing \todo{date, at least a year, maybe month} of this thesis, the specification of HTTP/3 is still at the draft stage, but only small details of the
...
figure~\ref{fig:http2-vs-http3-stack}. \todo{check numbering of the figures}
...
\begin{itemize}
  \item \textit{Stream multiplexing} --- \todo{check dash length}
  ...
  \item \textit{Faster connection establishment} ---
    QUIC intertwines\todo{maybe find a word that does not require dictionary} TCP-like three-way-handshake and TLS 1.3 handshake together, requiring fewer
    ...
    packet. \todo{explain a bit, reference the link, so that it is clear why we mention it}

  \item \textit{Always encrypted} ---
    Because TLS handshake is part of connection establishment \todo{the explanation of how TLS fits into QUIC needs to be clearer make sure it is clear that QUIC integrates TLS, and not replaces it}, the QUIC connection is always
  ...
  \item \textit{Separating connection identification from peer's IP address} ---
    QUIC protocol identifies the connection using its ID \todo{where does the ID come from?} rather than the peer's address. This
    feature is generally useful for mobile devices, \todo{mention IP addresses: which can hop between multiple IP addresses, which happens when...} which can often switch networks between Wi-Fi and
    mobile internet connection \todo{is it called GSM?}. With QUIC, these devices do not require
    reconnecting, but the connection can be migrated transparently from the perspective of the application protocol\todo{reword so that it is clear that it does not migrate \textbf{into} application protocol}.
    ...
    throughput. \todo{https://multipath-quic.org/}

\end{itemize}

\todo{does google still have/use the gQUIC implementation? What does Chrome use?}

Even though the QUIC specification is not yet finalized, there are multiple implementations of the
draft standard versions \todo{reformulate to make clear which word is adjective}. Many of these
...
\todo{write here: the goal is to further the implementation by... and than short analysis follows, see recording.}
