...
\begin{myFigure}
  ...
  \input{img/01-https-handshake.pdf_tex}
...
of \class{HttpClient} automatically chooses the best available HTTP version~\cite{HttpClientDocs}.
However, since QUIC can be used to build other protocols than HTTP/3, its implementation will be
exposed via public classes residing most likely inside the \namespace{System.Net.Quic} namespace.
...
\begin{itemize}
    ...
    \litem{Overhead of native calls} When execution transitions between managed and unmanaged code,
...
compare their performance to that of TCP+TLS based \class{SslStream}.
...
\begin{enumerate}
  ...
  \item Make the existing QUIC API public, and allow switching between the new managed
    implementation and the existing \libmsquic{} based one. \todo{Good point from ziki: this is not
    a goal, but a step. maybe call this section Steps?}
